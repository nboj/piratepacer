...
from src.PiratePacer.hooks.camera import Cam
from src.PiratePacer.hooks.quest import Quest
from src.PiratePacer.hooks.player import Player
from src.PiratePacer.hooks.player_model import PlayerModel
...
class Client(): #Thanks to Starrfox and wizwalker for most of the methods here. We're intentionally mimicking how WW handles clients just to give some parity.
    ...
    async def teleport(self, xyz: XYZ, move_after: bool = True, log: bool = True):
        ...
        await self.send_key(Keycode.W, 0.01)
